...
window.__wireframeState = state;
window.__wireframeDefaultPage = DEFAULT_PAGE;
  ...
  return `
    ...
    <form onsubmit="submitLancamento(event)">
      ...
      ${projs.map((p, i) => {
        ...
        <div class="lancamento-card">
          ...
          <div class="form-group" style="margin-top:var(--space-md);margin-bottom:0;">
            <label class="form-label required" for="just-${i}">Justificativa do lançamento</label>
      ...
      <div class="alert alert-warning" style="margin-top:var(--space-lg);">
        <i data-lucide="alert-triangle"></i>
        <span><strong>Atenção:</strong> Após confirmar, o lançamento será validado automaticamente e registrado de forma definitiva no histórico.</span>
      </div>

      <div style="display:flex;gap:var(--space-sm);margin-top:var(--space-md);">
        <button type="submit" class="btn btn-primary btn-lg">
          <i data-lucide="check-circle"></i> Registrar Horas do Mês
        </button>
...
function pageConfirmacaoLancamento() {
  ...
  return `
    <div class="success-page">
      ...
      <h1 class="success-title">Horas do mês registradas!</h1>
      <p class="success-sub">Seu lançamento de horas de <strong>${mesLabel}</strong> foi validado automaticamente e registrado com sucesso. Os dados já estão disponíveis no histórico e nos relatórios gerenciais.</p>
      <div style="display:flex;gap:var(--space-md);">
        <button class="btn btn-primary btn-lg" onclick="navigate('dashboard-colaborador')">
          <i data-lucide="layout-dashboard"></i> Voltar ao Painel
        </button>
